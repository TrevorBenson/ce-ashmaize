// Phase 6: Batch Size Re-Verification
// Re-test batch sizes around 131k and explore higher ranges
// Test Duration: 20s per config
...
fn main() {
    ...
    #[cfg(feature = "cuda")]
    {
        println!("=== Phase 6: Batch Size Re-Verification ===\n");
        ...
        // Test batch sizes around and above the previous optimal (131k)
        let batch_sizes = vec![
            (65_536, "65k (previous good)"),
            (98_304, "98k (between 65k and 131k)"),
            (131_072, "131k (previous optimal)"),
            (163_840, "164k (25% above optimal)"),
            (196_608, "197k (50% above optimal)"),
            (262_144, "262k (previous tested, slight degradation)"),
        ];
        
        println!("Testing {} batch size configurations...\n", batch_sizes.len());
        println!("{}", "=".repeat(70));
        ...
        println!("{:<12} {:<25} {:>15} {:>12} {:>12}",
            "Batch/GPU", "Label", "Hashrate (H/s)", "vs Baseline", "vs Peak");
        println!("{}", "-".repeat(80));
        
        for (batch, label, hashrate) in &results {
            let vs_baseline = ((hashrate - baseline) / baseline) * 100.0;
            let vs_peak = (hashrate / max_hashrate) * 100.0;
            let marker = if *hashrate == max_hashrate { " ✅" } else { "" };
            
            println!("{:<12} {:<25} {:>15.2} {:>11.1}% {:>11.1}%{}",
                batch, label, hashrate, vs_baseline, vs_peak, marker);
        }
        ...
        if *best_batch == 131_072 {
            println!("\n✅ Confirmed: 131,072 batch/GPU remains optimal");
        } else {
            println!("\n⚠️  New optimal found: {} batch/GPU (was 131,072)", best_batch);
        }
    }
}
