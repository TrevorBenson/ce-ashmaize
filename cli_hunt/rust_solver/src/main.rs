    ...
    match solver_mode {
        ...
        SolverMode::Auto => {
            let has_gpu = match gpu::CudaAshmaize::new() {
                Ok(_) => {
                    match gpu::CudaAshmaize::get_device_count() {
...
#[cfg(feature = "cuda")]
fn solve_multi_gpu(rom: &Rom, suffix: &str, difficulty_mask: u32, batch_size: usize) -> Option<u64> {
    use std::sync::mpsc;
    use std::thread;
    ...
    for gpu_id in 0..gpu_count {
        match gpu::CudaAshmaize::new_with_device(gpu_id) {
            Ok(c) => {
                eprintln!("GPU {}: {}", gpu_id, c.get_device_info().unwrap_or_else(|_| format!("GPU {}", gpu_id)));
                gpus.push(Arc::new(c));
            }
            Err(e) => {
                eprintln!("Failed to init GPU {}: {:?}, falling back to CPU", gpu_id, e);
                solve_cpu_only(rom, suffix, difficulty_mask);
                return None;
            }
        }
...
fn solve_mixed(rom: &Rom, suffix: &str, difficulty_mask: u32, batch_size: usize) -> Option<u64> {
    ...
    let _init_cuda = match gpu::CudaAshmaize::new() {
        Ok(_) => {},
        Err(e) => {
            eprintln!("Failed to initialize CUDA: {:?}, falling back to CPU only", e);
            solve_cpu_only(rom, suffix, difficulty_mask);
            return None;
        }
    };
    ...
    eprintln!("Mixed mode: {} CPU threads + {} GPU(s)", NUM_THREADS, gpu_count);
